...
public class NodeTest extends AbstractKattaTest {
	...
	public void testShardStatusSuccess() throws Exception {
		...
		katta.addIndex("index", "src/test/testIndexA/", StandardAnalyzer.class
				.getName(), 1);
		...
		assertEquals(IndexMetaData.IndexState.DEPLOYED, indexMetaData
				.getState());
	...
	public void testShardStatusNoSuccessNoIndexGiven() throws Exception {
		...
		// deploy index
		Katta katta = new Katta();
		katta.addIndex("index", "src/test/testIndexNotHere/",
		...
		IndexMetaData indexMetaData = new IndexMetaData();
		zkClientMaster.readData(indexPath, indexMetaData);
		assertEquals(IndexMetaData.IndexState.DEPLOY_ERROR, indexMetaData
				.getState());
	...
	public void testDeployShardAfterRestart() throws Exception {
		...
		// deploy index
		assertEquals(0, node.getDeployShards().size());
		Katta katta = new Katta();
		katta.addIndex("index", "src/test/testIndexA/", StandardAnalyzer.class
				.getName(), 1);

		// test
		assertTrue(node.getDeployShards().size() > 0);
		final String indexPath = ZkPathes.INDEXES + "/index";
		IndexMetaData indexMetaData = new IndexMetaData();
		zkClientMaster.readData(indexPath, indexMetaData);
		assertEquals(IndexMetaData.IndexState.DEPLOYED, indexMetaData
