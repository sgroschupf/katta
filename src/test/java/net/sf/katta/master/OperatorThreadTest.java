...
public class OperatorThreadTest extends AbstractLeaderTest {

  @Test(timeout = 10000)
  public void testSafeMode() throws Exception {
    DistributedBlockingQueue queue = mock(DistributedBlockingQueue.class);
  ...
  public void testOnNodeOperationCompletion() throws Exception {
    Node node = mockNode();
    DistributedBlockingQueue<NodeOperation> nodeQueue = publisNode(node);
  ...
  public void testOperationLocks_SuspendLockedTask() throws Exception {
    ...
    runLockSituation(queue, leaderOperation1, leaderOperation2, lockInstruction);
    verify(leaderOperation2, times(0)).execute(_context);
    verify(queue, times(1)).offer(leaderOperation2);
  }

  private void runLockSituation(DistributedBlockingQueue queue, final LeaderOperation leaderOperation1,
