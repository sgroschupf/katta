...
public class NonBlockingSetInt extends AbstractSet<Integer> implements Serializable {
  ...
  public void clear() {
    ...
    while (!CAS_nbsi(_nbsi, cleared))
      // Spin until clear works
      ;
  }
  ...
  private static final class NBSI {
    ...
    private transient final AtomicInteger _copyIdx; // Used to count bits
                                                    // started copying
    private transient final AtomicInteger _copyDone; // Used to count words
                                                      // copied in a resize
                                                      // operation
    private transient final int _sum_bits_length; // Sum of all nested
                                                  // _bits.lengths
    ...
    // are all set, we shift them off and recursively operate on the _nbsi64
    // set.
    ...
    private NBSI install_larger_new_bits(final int i) {
      if (_new == null) {
        ...
        // Note: must grow by exact powers-of-2 or the by-64-bit trick doesn't
        // work right
    ...
    private NBSI help_copy() {
      ...
      final int HELP = 8; // Tuning number: how much copy pain are we willing to
                          // inflict?
      // We "help" by forcing individual bit indices to copy. However, bits
      // come in lumps of 64 per word, so we just advance the bit counter by
      // 64's.
      int idx = top_nbsi._copyIdx.getAndAdd(64 * HELP);
      for (int i = 0; i < HELP; i++) {
        int j = idx + i * 64;
        j %= (top_nbsi._bits.length << 6); // Limit, wrap to array size; means
                                            // we retry indices
    ...
    private NBSI help_copy_impl(int i) {
      ...
      if (bits != mask(63)) { // Non-zero in old?
        ...
        if (new_bits == 0) { // New array is still zero
          new_bits = bits & ~mask(63); // Desired new value: a mutable copy of
                                        // bits
