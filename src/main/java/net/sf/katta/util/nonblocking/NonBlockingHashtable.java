...
public class NonBlockingHashtable<TypeK, TypeV> extends Dictionary<TypeK, TypeV> implements
  ...
  /**
   * Atomically do a {@link #remove(Object)} if-and-only-if the key is mapped to
   * a value which is <code>equals</code> to the given value.
   * 
   * @throws NullPointerException
   *             if the specified key or value is null
   */
  ...
  private static boolean keyeq(Object K, Object key, int[] hashes, int hash, int fullhash) {
    return K == key || // Either keys match exactly OR
        ...
        // new key/value pair.
        ((hashes[hash] == 0 || hashes[hash] == fullhash) && key.equals(K)); // Finally
                                                                            // do
                                                                            // the
                                                                            // hard
                                                                            // match
  ...
  private static final Object get_impl(final NonBlockingHashtable topmap, final Object[] kvs, final Object key) {
    ...
    final CHM chm = chm(kvs); // The CHM, for a volatile read below; reads slot
                              // 0 of kvs
    final int[] hashes = hashes(kvs); // The memoized hashes; reads slot 1 of
                                      // kvs
    ...
    while (true) {
      ...
      final Object K = key(kvs, idx); // Get key before volatile read, could be
                                      // null
      final Object V = val(kvs, idx); // Get value before volatile read, could
      ...
      if (key == TOMBSTONE) // found a TOMBSTONE key, means no more keys in this
                            // table
        return get_impl(topmap, topmap.help_copy(newkvs), key); // Retry in the
                                                                // new table
      ...
      if (keyeq(K, key, hashes, idx, fullhash)) {
        ...
        return get_impl(topmap, chm.copy_slot_and_check(topmap, kvs, idx, key), key); // Retry
                                                                                      // in
                                                                                      // the
                                                                                      // new
                                                                                      // table
  ...
  private static final Object putIfMatch(final NonBlockingHashtable topmap, final Object[] kvs, final Object key,
    ...
    if (expVal != NO_MATCH_OLD && // Do we care about expected-Value at all?
        ...
        (expVal != MATCH_ANY || V == TOMBSTONE || V == null) && !(V == null && expVal == TOMBSTONE) && // Match
                                                                                                        ...
                                                                                                        // null/TOMBSTONE
                                                                                                        // combo
        (expVal == null || !expVal.equals(V))) // Expensive equals check at the
                                                // last
  ...
  private static final class CHM<TypeK, TypeV> {
    ...
    private final AtomicReferenceFieldUpdater<CHM, Object[]> _newkvsUpdater = AtomicReferenceFieldUpdater.newUpdater(
        CHM.class, Object[].class, "_newkvs");
    ...
    private final Object[] resize(NonBlockingHashtable topmap, Object[] kvs) {
      ...
      if (newsz <= oldlen && // New table would shrink or hold steady?
          tm <= topmap._last_resize_milli + 10000 && // Recent resize (less
                                                      // than 1 sec ago)
      ...
      newkvs = new Object[((1 << log2) << 1) + 2]; // This can get expensive
                                                    // for big arrays
    ...
    private final void help_copy_impl(NonBlockingHashtable topmap, Object[] oldkvs, boolean copy_all) {
      ...
      while (_copyDone < oldlen) { // Still needing to copy?
        ...
        for (int i = 0; i < MIN_COPY_WORK; i++)
          if (copy_slot(topmap, (copyidx + i) & (oldlen - 1), oldkvs, newkvs)) // Made
                                                                                // an
                                                                                // oldtable
                                                                                // slot
                                                                                // go
        ...
        if (workdone > 0) // Report work-done occasionally
          copy_check_and_promote(topmap, oldkvs, workdone);// See if we can
                                                            // promote
        // for( int i=0; i<MIN_COPY_WORK; i++ )
        // if( copy_slot(topmap,(copyidx+i)&(oldlen-1),oldkvs,newkvs) ) // Made
        // an oldtable slot go dead?
    ...
    private final void copy_check_and_promote(NonBlockingHashtable topmap, Object[] oldkvs, int workdone) {
      ...
      if (copyDone + workdone == oldlen && // Ready to promote this table?
        ...
        topmap._last_resize_milli = System.currentTimeMillis(); // Record resize
                                                                // time for next
                                                                // check
        // long nano = System.nanoTime();
    ...
    private boolean copy_slot(NonBlockingHashtable topmap, int idx, Object[] oldkvs, Object[] newkvs) {
      ...
      while (!(oldval instanceof Prime)) {
        ...
        if (CAS_val(oldkvs, idx, oldval, box)) { // CAS down a box'd version of
                                                  // oldval
   ...
   * The {@link NonBlockingHashtable} does not normally create or using
   * {@link java.util.Map.Entry} objects so they will be created soley to
   * support this iteration. Iterating using {@link #keySet} or {@link #values}
