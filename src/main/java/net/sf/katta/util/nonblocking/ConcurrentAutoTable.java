 ...
 * 
 * <p>
 * This API is overkill for simple counters (e.g. no need for the 'mask') and is
 * untested as an API for making a scalable r/w lock and so is likely to change!
 * 
 * @since 1.5
 * @author Cliff Click
 */
...
public class ConcurrentAutoTable implements Serializable {
   ...
   * striped across an array of counters to avoid cache contention and has been
   * tested with performance scaling linearly up to 768 CPUs.
   */
  ...
  private static final AtomicReferenceFieldUpdater<ConcurrentAutoTable, CAT> _catUpdater = AtomicReferenceFieldUpdater
      .newUpdater(ConcurrentAutoTable.class, CAT.class, "_cat");
  ...
  private static final int hash() {
    ...
    h ^= (h >>> 7) ^ (h >>> 4);
    return h << 2; // Pad out cache lines. The goal is to avoid cache-line
                    // contention
  }
  ...
  private static class CAT implements Serializable {
    ...
    static private final AtomicLongFieldUpdater<CAT> _resizerUpdater = AtomicLongFieldUpdater.newUpdater(CAT.class,
        "_resizers");
    ...
    public long add_if_mask(long x, long mask, int hash, ConcurrentAutoTable master) {
      ...
      // Too much contention; double array size in an effort to reduce
      // contention
      long r = _resizers;
