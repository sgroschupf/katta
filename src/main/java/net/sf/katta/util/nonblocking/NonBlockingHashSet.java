...
public class NonBlockingHashSet<E> extends AbstractSet<E> implements Serializable {
   ...
   * IllegalStateException. Existing mutator calls in other threads racing with
   * this thread and will either throw IllegalStateException or their update
   * will be visible to this thread. This implies that a simple flag cannot make
   * the Set immutable, because a late-arriving update in another thread might
   * see immutable flag not set yet, then mutate the Set after the
   * {@link #readOnly} call returns. This call can be called concurrently (and
   * indeed until the operation completes, all calls on the Set from any thread
   * either complete normally or end up calling {@link #readOnly} internally).
   * 
   * <p>
   ...
   * after construction the Set is only read. Making the Set read-only will
   * cause updates arriving after construction is supposedly complete to throw
