 ...
 * 
 * <p>
 * <strong>The main benefit of this class</strong> over using plain {@link
 ...
 * <em>automatic</em>, it is easy to accidentally cause auto-boxing and
 * negate the space and speed benefits.
 * 
 * <p>
 ...
 * reflect insertion or removal of only some entries. Similarly, Iterators and
...
public class NonBlockingHashMapLong<TypeV> extends AbstractMap<Long, TypeV> implements ConcurrentMap<Long, TypeV>,
  ...
  /**
   * Create a new NonBlockingHashMapLong with initial room for the given number
   ...
   * {@code true} optimizes for space and is the default. {@code false}
   * optimizes for speed and doubles space costs for roughly a 10% speed
   * improvement.
   */
  ...
  /**
   * Create a new NonBlockingHashMapLong, setting both the initial size and the
   * space-for-speed tradeoff. {@code true} optimizes for space and is the
  ...
  /**
   * Atomically, do a {@link #put} if-and-only-if the key is not mapped. Useful
   * to ensure that only a single mapping for the key exists, even if many
   * threads are trying to create the mapping in parallel.
   * 
  ...
  /**
   * Atomically do a {@link #remove(long)} if-and-only-if the key is mapped to a
  ...
  private final TypeV putIfMatch(long key, Object newVal, Object oldVal) {
    ...
    if (key == NO_KEY) {
      ...
      if (oldVal == NO_MATCH_OLD || // Do we care about expected-Value at all?
          ...
          (oldVal == MATCH_ANY && curVal != TOMBSTONE) || oldVal.equals(curVal)) // Expensive
                                                                                  // equals
                                                                                  // check
        CAS(_val_1_offset, curVal, newVal); // One shot CAS update attempt
      return curVal == TOMBSTONE ? null : (TypeV) curVal; // Return the last
                                                          // value present
    }
  ...
  private static final class CHM<TypeV> implements Serializable {
    ...
    private static final AtomicReferenceFieldUpdater<CHM, CHM> _newchmUpdater = AtomicReferenceFieldUpdater.newUpdater(
        CHM.class, CHM.class, "_newchm");
    ...
    private final Object get_impl(final long key) {
      ...
      int reprobe_cnt = 0;
      while (true) {
        final long K = _keys[idx]; // Get key before volatile read, could be
                                    // NO_KEY
        final Object V = _vals[idx]; // Get value before volatile read, could
                                      // be null or Tombstone or Prime
        if (K == NO_KEY)
        ...
        if (key == K) {
          ...
          return copy_slot_and_check(idx, key).get_impl(key); // Retry in the
                                                              // new table
        }
    ...
    private final Object putIfMatch(final long key, final Object putval, final Object expVal) {
      ...
      if (expVal != NO_MATCH_OLD && // Do we care about expected-Value at all?
          ...
          (expVal != MATCH_ANY || V == TOMBSTONE || V == null) && !(V == null && expVal == TOMBSTONE) && // Match
                                                                                                          ...
                                                                                                          // null/TOMBSTONE
                                                                                                          // combo
          (expVal == null || !expVal.equals(V))) // Expensive equals check at
                                                  // the last
    ...
    private final CHM resize() {
      // Check for resize already in progress, probably triggered by another
      // thread
      ...
      if (_nbhml._opt_for_space) {
        // This heuristic leads to a much denser table with a higher reprobe
        // rate
      ...
      if (newsz <= oldlen && // New table would shrink or hold steady?
          tm <= _nbhml._last_resize_milli + 10000 && // Recent resize (less
                                                      // than 1 sec ago)
      ...
      if (newsz < oldlen)
        newsz = oldlen;
      // System.out.println("old="+oldlen+" new="+newsz+" size()="+sz+"
      // est_slots()="+q+" millis="+(tm-_nbhml._last_resize_milli));
      ...
      if (CAS_newchm(newchm)) { // NOW a resize-is-in-progress!
        ...
        // System.out.println(" "+nano+" Resize from "+oldlen+" to "+(1<<log2)+"
        // and had "+(_resizers-1)+" extras" );
    ...
    private final void help_copy_impl(final boolean copy_all) {
      ...
      while (_copyDone < oldlen) { // Still needing to copy?
        ...
        for (int i = 0; i < MIN_COPY_WORK; i++)
          if (copy_slot((copyidx + i) & (oldlen - 1))) // Made an oldtable slot
                                                        // go dead?
            workdone++; // Yes!
        if (workdone > 0) // Report work-done occasionally
          copy_check_and_promote(workdone);// See if we can promote
        // for( int i=0; i<MIN_COPY_WORK; i++ )
        // if( copy_slot((copyidx+i)&(oldlen-1)) ) // Made an oldtable slot go
        // dead?
    ...
    private final CHM copy_slot_and_check(int idx, Object should_help) {
      ...
      // Generically help along any copy (except if called recursively from a
      // helper)
      if (should_help != null)
        _nbhml.help_copy();
      return _newchm;
    ...
    private final void copy_check_and_promote(int workdone) {
      ...
      if (workdone > 0) {
        ...
        // System.out.print("
        // "+nowDone*100/oldlen+"%"+"_"+(_copyIdx*100/oldlen)+"%");
      }
      ...
      if (nowDone == oldlen && // Ready to promote this table?
        ...
        _nbhml._last_resize_milli = System.currentTimeMillis(); // Record resize
                                                                // time for next
                                                                // check
        // long nano = System.nanoTime();
        // System.out.println(" "+nano+" Promote table "+oldlen+" to
        // "+_newchm._keys.length);
    ...
    private boolean copy_slot(int idx) {
      ...
      while ((key = _keys[idx]) == NO_KEY)
        CAS_key(idx, NO_KEY, (idx + _keys.length)/*
                                                   * a non-zero key which hashes
                                                   * here
                                                   */);
  ...
  private class SnapshotV implements Iterator<TypeV>, Enumeration<TypeV> {
    ...
    private int _idx; // -2 for NO_KEY, -1 for CHECK_NEW_TABLE_LONG,
                      // 0-keys.length
    ...
    public TypeV next() {
      ...
      while (_idx < length()) { // Scan array
        _nextK = key(_idx++); // Get a key that definitely is in the set (for
                              // the moment!)
  ...
  /**
   * Returns a {@link Set} view of the keys contained in this map; with care the
   * keys may be iterated over <strong>without auto-boxing</strong>. The set is
   ...
   * The {@link org.cliffc.high_scale_lib.NonBlockingHashMap} does not normally
   * create or using {@link java.util.Map.Entry} objects so they will be created
   * soley to support this iteration. Iterating using {@link #keySet} or
   * {@link #values} will be more efficient. In addition, this version requires
