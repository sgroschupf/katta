...
public class Katta {
		...
		if (_zkClient.exists(indexPath)) {
			...
			try {
				...
				addIndex(indexName, indexMetaData.getPath(), indexMetaData
						.getAnalyzerClassName(), indexMetaData
						.getReplicationLevel());
			} catch (InterruptedException e) {
				LOG.error("Redeployment of index '" + indexName
						+ "' interrupted.");
			}
		} else {
	...
	public void listNodes() throws KattaException {
		...
		for (final String node : nodes) {
			...
			table.addRow(new String[] { nodeMetaData.getName(),
					nodeMetaData.getStartTimeAsDate(), "" + inService,
		...
		table.setHeader(new String[] {
				...
				"Start time",
				"Healthy",
				"Status (" + inServiceNodeCount + "/" + nodes.size()
						+ " nodes in Service)", "Starting" });
		System.out.println(table.toString());
	}

	public void listIndex() throws KattaException {
		final Table t = new Table(new String[] { "Name", "Deployed",
				"Analyzer", "Path" });
		...
		for (final String index : indexes) {
			...
			t.addRow(new String[] { index, metaData.getState().toString(),
					metaData.getAnalyzerClassName(), metaData.getPath() });
			// maybe show shards
	...
	public void addIndex(final String name, final String path,
			final String analyzerClass, final int replicationLevel)
		...
		_zkClient.create(indexPath, new IndexMetaData(path, analyzerClass,
				replicationLevel, IndexMetaData.IndexState.ANNOUNCED));
	...
	public static void search(final String[] indexNames,
			final String queryString, final int count) throws KattaException {
		...
		System.out.println(hits.size() + " hits found in "
				+ ((end - start) / 1000.0) + "sec.");
		int index = 0;
		final Table table = new Table(new String[] { "Hit", "Node", "Shard",
				"DocId", "Score" });
		for (final Hit hit : hits.getHits()) {
			table.addRow(new String[] { "" + index, hit.getNode(),
					hit.getShard(), "" + hit.getDocId(), "" + hit.getScore() });
			index++;
		}
		System.out.println(table.toString());
	}
	...
	public static void search(final String[] indexNames,
			final String queryString) throws KattaException {
		final IClient client = new Client();
		final IQuery query = new Query(queryString);
		final long start = System.currentTimeMillis();
		final int hitsSize = client.count(query, indexNames);
		final long end = System.currentTimeMillis();
		System.out.println(hitsSize + " Hits found in "
				+ ((end - start) / 1000.0) + "sec.");
	}
	...
	private static void printUsageAndExit() {
		...
		System.err
				.println("\tshowStructure\tShows the structure of a Katta installation.");
		System.err
				.println("\tremoveIndex <index name>\tRemove a index from a Katta installation.");
		System.err
				.println("\taddIndex <index name> <path to index> <lucene analyzer class> [<replication level>]\tAdd a index to a Katta installation.");
		System.err
				.println("\tlistErrors <index name>\tLists all deploy errors for a specified index.");
		System.err
				.println("\tredeployIndex <index name>\tTries to deploy an index.");
		System.exit(1);
	}
	...
	private static class Table {
		...
		public String toString() {
			...
			for (int i = 0; i < _header.length; i++) {
				...
				builder.append(column
						+ getChar(columnSizes[i] - column.length(), " ")
						+ " | ");
			}
			builder.append("\n=");
			...
			for (final String[] row : _rows) {
				...
				for (int i = 0; i < row.length; i++) {
					builder.append(row[i]
							+ getChar(columnSizes[i] - row[i].length(), " ")
							+ " | ");
				}
				builder.append("\n-");
				builder.append(getChar(rowWidth + columnSizes.length, "-")
		...
		private int[] getColumnSizes(final String[] header,
