...
import net.sf.katta.lib.lucene.query.WildcardQueryAndTermFilterWritable;
 ...
 */
public class LuceneClientTest extends AbstractLuceneIntegrationTest {
  ...
  public void testGetBinaryDetails() throws Exception {
    ...
    document.add(new Field(binaryFieldName, bytesFieldContent, storedFieldType));
    ...
    ILuceneClient client = new LuceneClient(_miniCluster.getZkConfiguration());
    final ILuceneQueryAndFilterWritable query = new TermQueryWritable(textFieldName, "sample");
  ...
  @Test
  public void testGetDetailsConcurrently() throws KattaException, InterruptedException {
    deployTestIndices(1, 1);
    ILuceneClient client = new LuceneClient(_miniCluster.getZkConfiguration());
    final ILuceneQueryAndFilterWritable query = new TermQueryWritable("content", "the");
    final Hits hits = client.search(query, new String[] { INDEX_NAME }, 10);
  ...
  public void testFieldSortWithNoResultShard() throws Exception {
    ...
    IndexWriterConfig config = new IndexWriterConfig(Version.LUCENE_41, new StandardAnalyzer(Version.LUCENE_41));
    config.setOpenMode(IndexWriterConfig.OpenMode.CREATE);
    IndexWriter indexWriter1 = new IndexWriter(FSDirectory.open(sortIndex1), config);
    IndexWriter indexWriter2 = new IndexWriter(FSDirectory.open(sortIndex2), config);

    Document document = new Document();
    document.add(new Field("text", "abc", Field.Store.YES, Index.NOT_ANALYZED));
    document.add(new LongField("timesort", 1234567890123l, Field.Store.YES));
    ...
    ILuceneClient client = new LuceneClient(_miniCluster.getZkConfiguration());
    Sort sort = new Sort(new SortField[] { new SortField("timesort", SortField.Type.LONG) });

    // query both documents
    ILuceneQueryAndFilterWritable query = new PrefixQueryWritable("text", "ab");
    Hits hits = client.search(query, new String[] { indexName }, 20, sort);
    assertEquals(2, hits.size());

    // query only one document
    query = new TermQueryWritable("text", "abc2");
  ...
  public void testSortedSearch() throws Exception {
    ...
    IndexWriterConfig config = new IndexWriterConfig(Version.LUCENE_41, new StandardAnalyzer(Version.LUCENE_41));
    config.setOpenMode(IndexWriterConfig.OpenMode.CREATE);
    IndexWriter indexWriter = new IndexWriter(FSDirectory.open(sortIndex), config);
    for (int i = 0; i < 20; i++) {
      Document document = new Document();
      ...
      document.add(new TextField(textFieldName, textField, Store.NO));
      indexWriter.addDocument(document);
    }
    indexWriter.forceMerge(1);
    indexWriter.close();
    ...
    // query and compare results
    ILuceneClient client = new LuceneClient(_miniCluster.getZkConfiguration());
    final ILuceneQueryAndFilterWritable query = new TermQueryWritable(textFieldName, queryTerm);
    Sort sort = new Sort(new SortField[] { new SortField("sortField", SortField.Type.INT) });
  ...
  @Test
  public void testSearchWithMissingShards() throws InterruptedException, KattaException {
    deployTestIndices(1, 1);
  ...
  public void testNumDocGreaterMaxInteger_KATTA_140() throws Exception {
    ...
    LuceneClient client = new LuceneClient(_miniCluster.getZkConfiguration()) {
      @Override
      protected DocumentFrequencyWritable getDocFrequencies(ILuceneQueryAndFilterWritable q, String[] indexNames) throws KattaException {
  ...
  public void testFilteredSearch() throws Exception {
    ...
    ILuceneClient client = new LuceneClient(_miniCluster.getZkConfiguration());
    final ILuceneQueryAndFilterWritable query = new WildcardQueryAndTermFilterWritable(textFieldName, "sample*3", filterFieldName, "3");

    final Hits hits = client.search(query, new String[] { filterIndex.getName() }, 100, null);
    assertNotNull(hits);
    List<Hit> hitsList = hits.getHits();
    for (final Hit hit : hitsList) {
      writeToLog("hit", hit);
